...
{
	var fns = [
	 	this.createVertexTemplateEntry('rhombus;whiteSpace=wrap;html=1;fillColor=#f09;2', 80, 50, ' ', '查询', null, null, 'diamond rhombus if condition decision conditional question test','Enquiry'),
	 	this.createVertexTemplateEntry('whiteSpace=wrap;html=1;fillColor=#90f;3', 50, 50, ' ', '动作', null, null, 'rect rectangle box','Action'),
 		this.createVertexTemplateEntry('ellipse;whiteSpace=wrap;html=1;fillColor=#f90;1', 50, 50, ' ', '决策', null, null, 'oval ellipse state','Decision'),
	 	this.createVertexTemplateEntry('rounded=1;whiteSpace=wrap;html=1;strockColor=#000;fillColor=rgba(0,0,0,0);;4', 120, 80, ' ', '计划', null, null, 'rounded rect rectangle box','Plan')
	];
...
Sidebar.prototype.addUmlPalette = function(expand)
{
	...
	var fns = [
		...
		{
	    	var cell = new mxCell('Go', new mxGeometry(0, 0, 50, 50),
	    		'ellipse;html=1;shape=startState;fillColor=#ccc;strokeColor=#ff0000;');
	    	cell.vertex = true;
	    	
			return sb.createVertexTemplateFromCells([cell], 30, 90, 'Start');
		}),
		this.createVertexTemplateEntry('ellipse;html=1;shape=endState;fillColor=#ccc;strokeColor=#ff0000;', 50, 50, 'End', 'End', null, null, 'uml activity state end'),
	];
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplateEntry = function(style, width, height, value,  title, showLabel, showTitle, tags,typeId)
{
// 		this.createVertexTemplateEntry('ellipse;whiteSpace=wrap;html=1;fillColor=#f90;', 50, 50, '决策', '决策1', '决策', null, null, 'oval ellipse state'),
//	tags = (tags != null && tags.length > 0) ? tags : title.toLowerCase();
	tags = (tags != null && tags.length > 0) ;
	
	return this.addEntry(tags, mxUtils.bind(this, function()
 	{
 		return this.createVertexTemplate(style, width, height, value,  title, showLabel, showTitle,null,typeId);
 	}));
}
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplate = function(style, width, height, value,  title, showLabel, showTitle, allowCellsInserted,typeId)
{
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
	// debugger
	if(typeId!=null&&""!=typeId){
		cells[0].setTypeId(typeId)
	}
	cells[0].vertex = true;
 ...
 * 创建默认属性
 */
Sidebar.prototype.addDefaultPropetyValue = function(graph){
    ...
    for(var i in cells){
        ...
        if(value == "查询"){
            obj.setAttribute('describe1',  '查询组件');
            obj.setAttribute('describe2',  '查询组件');
            obj.setAttribute('describe3',  '查询组件');
            obj.setAttribute('aa',  'ksdjfl');
        }
        if(value == "动作"){
            obj.setAttribute('describe',  '动作组件');
        }
        if(value == "决策"){
            obj.setAttribute('describe',  '决策组件');
        }
        if(value == "计划"){
            obj.setAttribute('describe',  '计划组件');
        }
        obj.setAttribute('label',  value);
        obj.setAttribute('labels',  value);
